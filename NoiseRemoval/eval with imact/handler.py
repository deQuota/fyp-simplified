...
def add_impact_to_forecast(event_set: pd.DataFrame, noise_filtered_forecast: pd.DataFrame):
    ...
    for index, row in noise_filtered_forecast.iterrows():
        for index_1, row_1 in event_set.iterrows():
            if row['Date'].strftime('%Y-%m-%d') == row_1['date'].strftime('%Y-%m-%d'):
...
df = remove_noise_of_raw(read_csv('kelani_valley.csv'))
fore_df: pd.DataFrame = prophet_forecast(df, num_days)

fore_df_nr: pd.DataFrame = noise_removal_forecat(read_csv('kelani_valley.csv'), fore_df)
...
event_set = pd.read_json('kelani_valley_iam.json')
impact_added_forecast = add_impact_to_forecast(event_set, fore_df_nr)


actual_df: pd.DataFrame = actual_df.join(fore_df_nr)

fore_df_without_nr = forecast_without_noise_filter(read_csv('kelani_valley.csv'), num_days)
...
print(mean_absolute_percentage_error(act_close['Close'], impact_added_forecast['Impact_Added']))
...
print(skl.mean_squared_error(act_close['Close'], impact_added_forecast['Impact_Added']))
...
print(skl.mean_absolute_error(act_close['Close'], impact_added_forecast['Impact_Added']))
...
print(skl.mean_squared_log_error(act_close['Close'], impact_added_forecast['Impact_Added']))
